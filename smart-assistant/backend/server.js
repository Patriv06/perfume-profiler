...
const { initDatabase, dbAll, dbRun, dbGet, widgetConfigCache } = require('./db');
...
app.get('/api/widget/config', async (req, res) => {
  ...
  try {
    ...
    const cachedConfig = widgetConfigCache.get(store_id);
    if (cachedConfig) {
      return res.json(cachedConfig);
    }

    const tenant = await dbGet('SELECT * FROM tenants WHERE store_id = $1', [store_id]);
    if (!tenant) {
      return res.status(404).json({ error: 'Tienda no registrada.' });
    }

    if (tenant.billing_status && tenant.billing_status !== 'active') {
      const disabledResponse = { status: 'disabled' };
      widgetConfigCache.set(store_id, disabledResponse);
    ...
    widgetConfigCache.set(store_id, responseData);
...
app.get('/api/recommend', async (req, res) => {
  ...
  try {
    ...
    // Get store details
    const tenant = await dbGet('SELECT store_url FROM tenants WHERE store_id = $1', [store_id]);
    if (!tenant) {
      return res.status(404).json({ error: 'Tienda no encontrada.' });
    }

    // Get all products for this store
    const products = await dbAll('SELECT * FROM products WHERE store_id = $1', [store_id]);
    ...
    const tags = await dbAll('SELECT * FROM product_tags WHERE store_id = $1', [store_id]);
...
app.post('/api/admin/config', async (req, res) => {
  ...
  try {
    ...
    if (category && category !== tenant.category) {
      ...
      if (store_id === '7732051' || store_id === '9999999' || !tenant.access_token || tenant.access_token.includes('mock')) {
        try {
          ...
          await dbRun('DELETE FROM product_tags WHERE store_id = $1', [store_id]);
          await dbRun('DELETE FROM products WHERE store_id = $1', [store_id]);
          ...
          if (template.mock_products && Array.isArray(template.mock_products)) {
            for (const p of template.mock_products) {
              const insertResult = await dbGet(
              ...
              if (insertResult && p.tags && Array.isArray(p.tags)) {
                const product_id = insertResult.id;
    ...
    await dbRun(
      'UPDATE tenants SET category = $1, quiz_config = $2 WHERE store_id = $3',
...
app.post('/api/admin/products/sync', async (req, res) => {
  ...
  try {
    ...
    for (const p of tnProducts) {
      ...
      const existingProduct = await dbGet('SELECT id FROM products WHERE store_id = $1 AND sku = $2', [store_id, sku]);
      ...
      if (existingProduct) {
        ...
        await dbRun(
          `UPDATE products 
           SET tiendanube_id = $1, variant_id = $2, canonical_url = $3, name = $4, description = $5, image_url = $6, price = $7
           WHERE id = $8`,
      ...
      await dbRun('DELETE FROM product_tags WHERE store_id = $1 AND product_id = $2', [store_id, product_id]);
...
app.post('/api/admin/products/tags', async (req, res) => {
  ...
  try {
    ...
    const product = await dbGet('SELECT id FROM products WHERE store_id = $1 AND id = $2', [store_id, product_id]);
    if (!product) {
